...
> :arrow_double_up:[Top](#1-data-structure)    :leftwards_arrow_with_hook:[Back](https://github.com/WeareSoft/tech-interview#1-data-structure)    :information_source:[Home](https://github.com/WeareSoft/tech-interview#tech-interview)
* 완전이진트리의 일종으로 우선순위큐를 위하여 만들어진 자료구조
* 여러개의 값들 중 최솟값이나 최댓값을 빠르게 찾아내기 위한 구조
* 힙 트리에서는 중복된 값을 허용한다.
* 힙의 원소 추가
  - 인덱스 순으로 노드의 마지막에 원소를 추가한다.
  - 부모 노드와 비교하며 교환하여 힙의 성질을 만족한다.
* 힙의 원소 삭제
  - 최대 힙에서 루트 노드가 최대값이므로 루트 노드를 삭제한다.
  - 삭제된 루트노드에서 마지막 힙의 원소를 가져온다.
  - 힙을 재구성한다.
