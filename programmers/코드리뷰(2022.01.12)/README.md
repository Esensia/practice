# Code Review
---
* 10:30에 화상참여를 누르니 바로 연결이 됨
* 리뷰를 담당하는 사람은 두명이 있고 자기소개를 해주심 시간관계상 답변을 끊을수도 있다고 먼저 말함
* 1번부터 시작.. 코드가 나와있음
* 문제마다 공통적인 질문은 어떻게 접근했는지? 무엇을 중요하게 생각했는지였음
* 리뷰 분위기는 좋았고 답변이 막히는 부분은 힌트를 주려고 하는 모습이 보였음
* 전체적인 질문에 대한 답변을 한것 같으나 횡설수설한 부분이 있었음
* 준비한대로 나오고 할수있는건 다했다..
---
### 1번 문제
...
```
  ...
  * 실제질문
    * Q. 문제의 접근방법?
      - 위 대로 답변
    * Q. 사람이 더 늘어날 경우 처리 방법은?
      - 상수값을 쓴것은 배열의 크기로 바꾸고 점수계산 방법은 동일하게 적용한다 답변
---
### 3번 문제
```java
import java.util.*;
...
```
  ...
  * 실제질문
    * Q. 문제의 접근방법?
      - 위 대로 답변
    * Q. 1,2,3번 동일하게 배열로 처리를 했는데 처리한 이유와 Collections의 종류나 특징에 대해
      - 문제에서 주어진 객체들이 배열이라 굳이 이걸 Collections 객체로 바꿔서 할 필요가 없으며
      - Collections의 종류에는 ArrayList, LinkedList가 있다고 답변함
      - 현직인데 map이나 set은 안쓰냐고 물어보길래
      - map은 key와 value로 이루어진 집합이고 set은 중복을 피하기 위해 쓴다고 답함
    * Q. 더 바꿔야 할 곳은 없는지? 배열의 indexOutOfRange 부분을 말씀하시는거 같았음
      - 제대로 답변 못함
---
### 4번 문제
```sql
...
```
  ...
  * 실제질문
    * Q. 문제의 접근방법은?
      - 위 대로 답변
    * Q. 조인을 썼는데 조인의 종류와 특징은?
      - join에 innerjoin과 outerjoin이 있는데 innerjoin은 null값을 허용하지 않고 outerjoin은 null을 허용한다고 답함
---
...
```
  ...
  * 실제질문
    * Q. 문제의 접근방법은?
      - 위 대로 답변
    * Q. DECODE를 썼는데 다른 방법은?
      - CASE WHEN을 쓰면 된다고 답변함
    * 통계적인 부분이 나오는 쿼리는 오라클에서 제공하는 함수가 있는데 아는것이 있는지?
      - prior to 함수를 쓰면 될것이라 답변함 (나중에 찾아보니 pivot함수가 있었네ㅎㅎㅎ)
    * 단순 문제해결을 위해 위와같이 작성을 했는지?
      - 그렇다고 답함
